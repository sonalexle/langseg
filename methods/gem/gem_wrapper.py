...
class GEMWrapper(nn.Module):
    ...
    def encode_text(self, text: list):
        ...
        self.text_embeddings = text_embedding.unsqueeze(0)

    def min_max(self, logits):
        B, num_prompt = logits.shape[:2]
        logits_min = logits.reshape(B, num_prompt, -1).min(dim=-1, keepdim=True)[0].unsqueeze(-1) + 1e-8
    ...
    def forward(
        self, image: torch.Tensor, text: list, normalize: bool = True,
        ...
        text_embeddings = self.text_embeddings # [1, num_prompt, dim]
        ...
        # Interpolate
        if output_size is not False:
            if output_size is None:
                output_size = (W, H)
            img_txt_matching = F.interpolate(img_txt_matching, size=output_size, mode='bilinear')  # [1, num_prompt, W, H]
